...
struct OFBasedDescriptorMO
{
	...
	virtual void Describe(DesInData &in, DesOutData &out)
	{
			...
			for (auto & imgPair : in.first){								// for each image
				for (int i = cuboid.xi; i <= cuboid.xf; ++i){				//for each row 
					for (int j = cuboid.yi; j <= cuboid.yf; ++j){			//for each pixel
						if (imgPair.second(i, j) > _thrMagnitude){			//if magnitude is large enough
...
static inline void VecDesp2Mat( std::vector<cv::Point2f> &vecPoints, std::vector<cv::Point2f> &positions, 
